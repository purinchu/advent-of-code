...
try {
    ...
    // The simulator will rely on being paired with a different simulated
    // hardware (a robot in this case). This needs cleaned up to use actual
    // signal channels but for now going to do this with callbacks

    let hull = new Map(); // maps "x,y" str to 0 or 1 for paint color
    let x = 0; // coords
    let y = 0;
    let min_x = 1, min_y = 1; // extents
    let max_x = -1, max_y = -1;
    let dir = '^';
    let expecting_color = true; // output is either a color or direction

    // preload with input based on cur pos
    const send_cur_color = () => {
        const key = `${x},${y}`;
        const color = hull.get(key) ?? 0;

//      console.log(`Sending ${color} to intcode program`);
        intcode.appendToInput(color);
    };
    ...
    const cb = (out) => {
        if (expecting_color) {
            ...
            // update Intcode on new paint value
            send_cur_color();
        }

        expecting_color = !expecting_color;
    };

    intcode.setOutputCallback(cb);

    hull.set('0,0', 1); // Preload with white square
    send_cur_color();

    const [res, out] = intcode.evaluate();

    print(`Result: ${res}. Num squares painted: ${hull.size}. Registration tag:`);
    ...
    for (let j = min_y; j <= max_y; j++) {
        ...
        for (let i = min_x; i <= max_x; i++) {
            const key = `${i},${j}`;
            row = row + (hull.get(key) ?? 0);
        }

        print (row.replaceAll('0', ' ').replaceAll('1', 'B'));
    }
