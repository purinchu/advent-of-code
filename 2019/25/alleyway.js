...
try {
    ...
    let hull = new Map(); // maps "x,y" str to [0..4] to represent game state
    ...
    const num_blks = Array.from(hull.values().filter(x => x == 2)).length;

    print(`Result: ${res}. Output: ${out}. Num blocks: ${num_blks}`);
} catch (err) {
