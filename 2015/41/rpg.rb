...
# 2015 Day 21 Part 1 / Puzzle 2015/41
...
class Puzzle
  ...
  def permute_loadouts
        ...
        it.each { |arm|
          [@rings.permutation(0), @rings.permutation(1), @rings.permutation(2)].each { |it2|
            it2.each { |rn|
              ...
              if fight_loadout(summary)
                if summary[1] < lowest_cost
                  lowest_cost = summary[1]
                  lowest_loadout = loadout
                end
              end
    ...
    puts "Lowest cost was #{lowest_cost} with #{lowest_loadout}"
  end
end
