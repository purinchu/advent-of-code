...
fn main() {
    ...
    // perimeter is interesting. What seems to work is to give every cell 4
    // sides and then subtract 1 for every neighbor...?
    let mut perimeters = region_areas.iter().map(|x| x * 4).collect::<Vec<_>>();
    for (idx, reg) in region_id.iter().enumerate() {
        let (x, y) = grid.pos_from_id(idx);
        let p_idx = region_ids.iter().position(|&r_id| r_id == *reg).unwrap();
        perimeters[p_idx] -= neighbors_of(&grid, x, y).len();
    }

    let total_price: usize = region_areas.iter().zip(perimeters.iter())
...
//  grid.dump_grid();
//  println!("");
//  let mut g2 = Grid{w: grid.w, h: grid.h, chars: region_id};
//  for c in &mut g2.chars {
//      *c = b'0' + *c;
//  }
//  g2.dump_grid();
}
