...
fn cost_for_end(g: &Grid, start: (usize, usize), end: (usize, usize)) -> i32 {
    let w = g.w;
    ...
    return g.visit_from(start.0, start.1, neighbors, visit);
}
...
fn main() {
    ...
    let mut remaining_bytes = lines.split_off(1024);
    for pair in lines {
        let pairv = pair
            .split(',')
            .map(str::parse::<usize>)
    ...
    for (i, pos) in remaining_bytes.iter().enumerate() {
        let pairv = pos
            .split(',')
            .map(str::parse::<usize>)
            .map(Result::unwrap)
        ...
        let cost = cost_for_end(&grid, start, end);
        if cost < 0 {
            println!("Unreachable after additional byte {}", i);
            println!("This is position: {}", pos);
