...
fn main() {
    ...
    let mut cycles_len: Vec<Option<i32>> = vec![None; positions.len()];
    let mut cycles_to_find = positions.len() as i32;
    ...
    for i in 1..=max_steps {
        for idx in 0..positions.len() {
            ...
            if cycles_len[idx].is_none() && positions[idx] == starts[idx] {
//              println!("Found cycle for {} at step {}", idx, i);
                cycles_to_find -= 1;
                cycles_len[idx] = Some(i);
            }
...
//      show_robots(&positions, w, h);
    ...
    println!("If that wasn't a tree, fix the safety score filter in the code");

    if cycles_to_find == 0 {
        assert!(cycles_len.iter().all(|e|e.is_some()));
    }
}
